...
# Claude Code status line script - Based on agnoster theme
# Shows: user@hostname | ~/project | main (2 files, synced 20m ago) | ████▄░░░░░ 45%
#
# Context calculation:
...
# Get user@hostname (agnoster style)
user=$(whoami)
host=$(hostname -s)
...
if [[ -n "$cwd" && -d "$cwd" ]]; then
    ...
    if [[ -n "$branch" ]]; then
        ...
        # Build git status string (agnoster style - concise)
        if [[ "$file_count" -eq 0 ]]; then
            git_status="(clean, ${sync_status})"
        elif [[ "$file_count" -eq 1 ]]; then
            git_status="(1 file, ${sync_status})"
        else
...
# Build output: user@host | dir | branch (status) | context (agnoster inspired)
output="${user}@${host} | ${dir}"
